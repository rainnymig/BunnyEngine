...
layout(set = 3, binding = 0) uniform sampler2D lightShadowMap;
...
void main()
{
    ...
    uvec4 lightShadowInfo = imageLoad(lightShadowMap, gl_FragCoord.xy);
    for (uint i = 0; i < lightCountCapped; i++)
    {
        float shadowCoef = (lightShadowInfo & (shadowBit << i)) ? 0.2 : 1.0;
